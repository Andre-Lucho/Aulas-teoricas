...
export function FlashcardModal({ flashcard, isOpen, onClose, onEdit, onDelete, onViewQuestion }) {
        ...
        <div className="relative">
          ...
          <DialogHeader className="p-6 pb-4">
            <div className="flex items-center justify-between">
              ...
              <Badge className={`${difficultyColors[flashcard.difficulty]} text-white`}>
              ...
              >
                ...
                <div className="absolute inset-0 w-full h-full backface-hidden bg-card border rounded-lg p-6 flex items-center justify-center">
                  <div className="text-center">
                    <h3 className="text-lg font-medium mb-4 text-muted-foreground">Pergunta</h3>
                    <p className="text-foreground text-base leading-relaxed">{flashcard.question}</p>
                  </div>
                </div>
                ...
                <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180 bg-primary/5 border rounded-lg p-6 flex items-center justify-center">
                  <div className="text-center">
                    <h3 className="text-lg font-medium mb-4 text-muted-foreground">Resposta</h3>
                    <p className="text-foreground text-base leading-relaxed">{flashcard.answer}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-center gap-2 p-6 pt-0 border-t">
            <Button onClick={() => onViewQuestion(flashcard.questionId)} variant="outline" size="sm" className="gap-2">
              <LuFileQuestion className="h-4 w-4" />
              Questão
            </Button>

            <Button onClick={() => onEdit(flashcard.id)} variant="outline" size="sm" className="gap-2">
