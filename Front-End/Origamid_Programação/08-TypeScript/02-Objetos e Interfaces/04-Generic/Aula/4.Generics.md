...
### Um tipo genérico é uma forma de declararmos um parâmetro para a nossa função, classe ou interface. Esse tipo poderá ser indicado no momento de setar da função através de <Tipo> ou<T>
...
### É possível indicar que tipo o genérico deve herdar de uma interface específica com o extends. <br>Isso deixa o código mais seguro, pois só poderemos usar métodos e propriedades da interface que o genérico herdou.
...
if (link) {
  ...
  // extractText<HTMLAnchorElement>(el: HTMLAnchorElement): string
}
