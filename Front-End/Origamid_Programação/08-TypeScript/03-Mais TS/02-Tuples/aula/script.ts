...
const produto1: [string, number] = ['Doom', 200];
...
function getText1(selector: string): [HTMLElement, string] | null {
  const elem = document.querySelector<HTMLElement>(selector);
  return elem ? [elem, elem.innerText] : null;
}
...
if (button1) button1[1].toLowerCase(); // --> já tenho os métodos de string
